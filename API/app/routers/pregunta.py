...
def leer_preguntas(skip: int = 0, limit: int = 10, session: Session = Depends(get_session)):
    preguntas = session.exec(select(Pregunta).offset(skip).limit(limit)).all()
    return preguntas
...
def actualizar_pregunta(pregunta_id: int, pregunta: Pregunta, session: Session = Depends(get_session)):
    ...
    if not db_pregunta:
        raise HTTPException(status_code=404, detail="Pregunta no encontrada")
    pregunta_data = pregunta.dict(exclude_unset=True)
